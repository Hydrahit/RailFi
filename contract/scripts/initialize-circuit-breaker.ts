...
async function main(): Promise<void> {
  ...
  const [circuitBreaker] = PublicKey.findProgramAddressSync([CIRCUIT_BREAKER_SEED], programId);

  console.log("\nRailPay Circuit Breaker Initialization");
  console.log(`RPC:             ${RPC_URL}`);
  console.log(`Program:         ${programId.toBase58()}`);
  console.log(`Admin:           ${payer.publicKey.toBase58()}`);
  console.log(`Protocol Config: ${protocolConfig.toBase58()}`);
  console.log(`Circuit Breaker: ${circuitBreaker.toBase58()}`);
...
main().catch((error: unknown) => {
  ...
  console.error(`\nCircuit breaker init failed: ${message}`);
  process.exit(1);
});
